...
export function api(path, { token, method = 'GET', body } = {}) {
  return fetch(`${API_ROOT}${path}`, {
    method,
    headers: { ...(token ? { Authorization: `Token ${token}` } : {}), ...(body ? { 'Content-Type': 'application/json' } : {}) },
    body: body ? JSON.stringify(body) : undefined,
  }).then(async response => {
    const data = await response.json().catch(() => ({}))
    if (!response.ok) {
      const message = typeof data.detail === 'string' ? data.detail : Object.entries(data).map(([key, value]) => `${key}: ${[].concat(value).join(' ')}`).join('；')
      throw new ApiError(message || '请求失败', { status: response.status, data })
    }
    return data
  })
}
...
export async function download(path, { token, body } = {}) {
  ...
  if (!response.ok) {
    const data = await response.json().catch(() => ({}))
    const message = typeof data.detail === 'string' ? data.detail : Object.entries(data).map(([key, value]) => `${key}: ${[].concat(value).join(' ')}`).join('；')
    throw new ApiError(message || '导出失败', { status: response.status, data })
  }
